...
use_cases = ("n2capture, CO2N2458, co2capture, ch4co2, h2selectivityco2, h2co2selectivity, n2capture_multi, co2capture_air, co2capture_combustion, orano, CEA_VALDUC")
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description=("SATT SAYENS ENTREPRISE'S PROJECT: "
                     "Optimization program aiding process engineer "
                     "for membrane system design separation"),
        epilog=("Further documentation is available "
                "by contacting <bernardetta.addis@loria.fr>."))

    parser.add_argument("--verbose",
                        action='store_true',
                        help=("Enable mind_api 's logging"))

    parser.add_argument("--debug",
                        action='store_true',
                        help="debug_mode : Display solvers's logging")
    ...
    parser.add_argument("--no_starting_point",
                        action='store_true',
                        help="""Starting point flag""")

    parser.add_argument("--no_simplified_model",
                        action='store_true',
                        help="""Simplified model flag""")

    parser.add_argument("--gams",
                        action='store_true',
                        help=("True if gams (General Algebraic Modeling "
                              "Language) solver available"))
    ...
    parser.add_argument("--visualise",
                        action='store_true',
                        help="visualise desing process's result")

    parser.add_argument("--opex",
                        action='store_true',
    ...
    parser.add_argument("--save_log_sol",
                        action='store_true',
...
def data_instance(case_study):
    ...
    instance['num_membranes'] = 2
    # instance['ub_area'] = [500, 500, 500]
    # instance['lb_area'] = [1, 1, 1]
    # instance['ub_acell'] = [10, 10, 10]

    instance['ub_area'] = [10000, 10000]
    instance['lb_area'] = [1, 1]
    instance['ub_acell'] = [1, 1]

    # instance['ub_area'] = [10000]
    # instance['lb_area'] = [1]
    # instance['ub_acell'] = [25]
    ...
    if case_study == 'CO2N2458':
        ...
        instance['fname_perm'] = (instance['data_dir'] + 'N2Capture_multiProfile_perm.dat')
        ...
        instance['fname_eco'] = (instance['data_dir'] + 'CO2Capture_eco.dat')
        instance['uniform_pup'] = True
        instance['vp'] = True
        instance['variable_perm'] =False
...
def printing_post_process_operation(model, parameter, obj_object, arg_capex=False, arg_opex=False, save_sol=False):
...
def load_configuration():
    ...
    instance_params = ["fname", "fname_perm", "fname_eco",
                       "num_membranes", "ub_area", "lb_area", "ub_acell",
                       "vp", "uniform_pup", "variable_perm","fixing_var"
                      ]
    config = configparser.ConfigParser()
    try:
        config.read('config.ini')
        if not ('tuning' in config.sections() and 'instance' in config.sections()):
            raise ValueError(f"Errror: config.ini is not set correctly, generate new one with option (--config)")
        for param in tuning_params:
            if param not in config['tuning']:
                raise ValueError(f"Eroor: {param} not defined in config.ini (section: tuning)")

        for param in instance_params:
            if param not in config['instance']:
                raise ValueError(f"Eroor: {param} not defined in config.ini (section: instance)")
    ...
    convert_dict = lambda type_, table : {
        x.split(":")[0][1:len(x.split(":")[0])-1]:type_(x.split(":")[1]) for x in table[1:len(table)-1].split(', ')}
    convert_bool = lambda name : True if name in ['true', 'True', 'on', 'ON', 'yes', 'YES'] else False
...
def main():
    ...
    if args.config:
        ...
        choice_of_solver = args.solver_name or 'knitroampl'
        ...
        my_solver = GlobalOptimisation(optsolver, instance['log_dir'],
                                       args.debug,
                                       not args.no_starting_point,
                                       not args.no_simplified_model)

        tuning['algo'] = "multistart" if args.algorithm_choice not in algorithms else args.algorithm_choice
        ...
        if my_solver.feasible:
            print()
            printing_post_process_operation(modelisation.instance,
        ...
        if my_solver.feasible and args.visualise:
            my_solver.visualise_solutions(modelisation.instance,
                                          modelisation.parameter, True)
        elif my_solver.feasible:
            my_solver.visualise_solutions(modelisation.instance,
                                          modelisation.parameter, False)
