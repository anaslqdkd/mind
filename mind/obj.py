...
class ObjFunction():
    ...
    def __init__(self, fname_eco, parameter, log_dir: str, loading=False):
        ...
        self.uf_cp = economic['UF_1968'] if 'UF_1968' in economic.keys(
        ) else None
        self.uf_exp = economic['UF_2000'] if 'UF_2000' in economic.keys(
        ) else None
        ...
        self.eta_cp = economic['eta_cp'] if 'eta_cp' in economic.keys(
        ) else 0.85
        self.eta_exp = economic['eta_exp'] if 'eta_exp' in economic.keys(
        ) else 0.85
        self.eta_vp_0 = economic['eta_vp_0'] if 'eta_vp_0' in economic.keys(
        ) else None
        self.eta_vp_1 = economic['eta_vp_1'] if 'eta_vp_1' in economic.keys(
        ) else None
        ...
        self.a = ((1 / self.membrane_frame_area_coef)**
                  self.alpha_mem) * self.kmf * ((1 / 55)**self.alpha_press_mem)
        self.b_cp = economic['C_cp'] * (
            1 / self.stage_compressor_area_coef)**self.beta_cp * (
                self.mpfc + self.mfc - 1) * self.uf_cp * self.k_er
        self.b_exp = economic['C_exp'] * (
            1 / self.stage_compressor_area_coef)**self.beta_exp * (
                self.mpfc + self.mfc - 1) * self.uf_exp * self.k_er

        self.bp_coef = economic['bp_coef'] if 'bp_coef' in economic.keys(
        ) else 1.12
        self.p_coef = economic['p_coef'] if 'p_coef' in economic.keys() else 0.2
        self.cmc_coef = economic['cmc_coef'] if 'cmc_coef' in economic.keys(
        ) else 0.05
        # classical
        # self.cmc_coef = economic['cmc_coef'] if 'cmc_coef' in economic.keys() else 0.03
        self.lti_coef = economic['lti_coef'] if 'lti_coef' in economic.keys(
        ) else 0.15
        self.loc_coef = economic['loc_coef'] if 'loc_coef' in economic.keys(
        ) else 1.15
        self.dl_coef = economic['dl_coef'] if 'dl_coef' in economic.keys(
        ) else 11
    ...
    def except_when_none_value(self):
        if self.r is None:
            raise ValueError(
                "process's objective function parameter 'R' is missing")
        if self.t is None:
            raise ValueError(
                "process's objective function parameter 'T' is missing")
        if self.t_op is None:
            raise ValueError(
                "process's objective function parameter 't_op' is missing")
        if self.phi is None:
            raise ValueError(
                "process's objective function parameter 'phi' is missing")
        if self.kmf is None:
            raise ValueError(
                "process's objective function parameter 'kmf' is missing")
        if self.mpfc is None:
            raise ValueError(
                "process's objective function parameter 'mpfc' is missing")
        if self.mfc is None:
            raise ValueError(
                "process's objective function parameter 'mfc' is missing")
        ...
        if self.cvp is None:
            raise ValueError(
                "process's objective function parameter 'c_vp' is missing")
        if self.cc is None:
            raise ValueError(
                "process's objective function parameter 'c_cp' is missing")
        ...
        if self.k_er is None:
            raise ValueError(
                "process's objective function parameter 'K_er' is missing")
        if self.k_el is None:
            raise ValueError(
                "process's objective function parameter 'K_el' is missing")
            ...
            for line in file:
                if line == "\n":
                    ...
                    try:
                        ...
                        raise AttributeError('Attributes {} not defined'.format(
                            line[0]))
                    else:
    ...
    def get_membrane_frame_cost(self, model):
        ...
        if self.parameter.uniform_pup:
            i_mfs = [
                self.a * (model.area[s]**self.alpha_mem) *
                (model.pressure_up**self.alpha_press_mem) for s in model.states
            ]
        else:
            i_mfs = [
                self.a * (model.area[s]**self.alpha_mem) *
    ...
    def power_compressing_permeate(self, model):
        ...
        for s in model.states:
            ...
            for s1 in model.states:
                ...
                wcp_s_factor2 = ((self.gamma * self.r * self.t) /
                                 (self.gamma - 1))
    ...
    def power_vacuum_pump(self, model):
        ...
        for s in model.states:
            if self.parameter.vp:
                flow = sum(model.splitPERM[s, s1]
                           for s1 in model.states) + model.splitOutPERM[s]
                pressure_ratio = 1.0 / model.pressure_down[s]

                #eta_vp = (self.eta_vp_1 * pe.log(model.pressure_down[s] / 1) +
                #          self.eta_vp_0)
                
                eta_vp = self.eta_vp_1 * self.eta_vp_0*model.pressure_down[s]/(1 + self.eta_vp_0*model.pressure_down[s])
    ...
    def get_stage_compressor_cost(self, model):
        ...
        return i_ccs, sum(
            wcps_perm[s - 1] + wcps_ret[s - 1] for s in model.states)
    ...
    def total_equipement(self, model):
        ...
        if model.pressure_prod.value == -1:
            ...
            return sum(self.ims[s - 1] + self.i_mfs[s - 1] + self.i_ccs[s - 1] + self.i_ccf[s - 1] +
                       self.i_vps[s - 1] for s in model.states)
        else:
            if model.ub_press_up.value <= model.pressure_prod.value:
                # compressor
                return sum(self.ims[s - 1] + self.i_mfs[s - 1] + self.i_ccs[s - 1] +
                           self.i_ccf[s - 1] + self.i_vps[s - 1]
                           for s in model.states) + self.i_cprod

            if model.lb_press_up.value >= model.pressure_prod.value:
                # expander
                return sum(self.ims[s - 1] + self.i_mfs[s - 1] + self.i_ccs[s - 1] +
                           self.i_ccf[s - 1] + self.i_vps[s - 1]
                           for s in model.states) + self.i_exp
    ...
    def annual_product_loss(self, model):
        ...
        return ((model.OUT_waste * self.loss_coef*self.t_op) *
                (model.XOUT_waste[model.final_product] /
    ...
    def specific_product_separation(self, model):
        ...
        """
        product_annual_separation = (model.OUT_prod *
                                     model.XOUT_prod[model.final_product] *
                                     model.molarmass[model.final_product] *1e-6*
                                     3600*self.t_op)
	...
	#return self.sc_prod
        ...
        return 1e3*self.ec/self.t_op/self.k_el/(model.OUT_prod *model.XOUT_prod[model.final_product])/(model.molarmass[model.final_product]*3.6) #kWh/tCO2
